...
Spring MVC Async vs WebFlux
전자는 요청당 쓰레드가 할당되고 쓰레드 컨텍스트 스위칭에 의해 지연이 발생할 수 있고, 논블로킹이지만
블로킹이 발생할 소지가 있습니다. 

후자는 적은 쓰레드, 이벤트드리븐 기반으로 스트림을 처리할 수 있어서 완벽한 비동기를 지원합니다.
지연이 많은 작업이나 동시 사용자가 많을 수록 후자의 동시성 처리 효율이 올라갑니다. 

조만간 동일한 프로젝트를 WebFlux 로 개발하여 올리겠습니다.
```
...
(현재 로그인한 사용자가 1천명이고 동시에 이 기능을 호출 한다고 가정해 볼 때) 
해당 기능은 1천개의 쓰레드가 필요할 것이고, 각각의 쓰레드는 1), 2), 3) 을 순차적으로 실행하게 될 것입니다. 
