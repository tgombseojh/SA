...
1씩 증가해 나갈 것입니다. "1씩 증가" 라는 작업은 기존 값을 읽어서 +1을
적용하는 select 와 update 가 하나인 작업을 의미합니다.  
...
3번 사용자에 의해 | 판교분식 | 2 | 라는 데이터가 생성될 것입니다. 

검색 횟수가 3이어야 할 데이터가 다시 한번 2가 되어버린 것은 
고정된 하나의 데이터에 여러 사용자가 동시에 접근하여 데이터를 수정하려 했기 때문입니다. 

문제 해결 방법 중 하나는 테이블 row 에 lock 을 걸고 select 와 update 를 하나의 작업으로 만드는 
transaction 을 설정하여 누군가가 데이터를 사용하고 있을때는 다른 사람이 사용할 수 없도록 하는 것입니다.
...
(대용량 트래픽을 전제로 제 생각을 작성하고 있을 뿐 해당 프로젝트 코드에 적용된 것은 아닙니다.)
해법) 메세지 전달 방식
검색 횟수 증가 작업을 요청 받는 A큐와, 증가 작업을 전담하여 처리하는 A쓰레드.
현재시점의 검색 횟수 조회 작업을 요청 받는 B큐와, 조회 작업을 전담하는 B쓰레드 하나를 만들어서 
수천의 요청자들이 요청을 날리고 다른 일을 할 수 있도록 합니다. 
...
- 많은 데이터가 저장된 DB
```
DB 조회 성능을 위한 인덱스 처리는 기본이라고 생각합니다. 

사용자들이 자주 찾는 데이터는 메모리 캐싱을 해두고, (매번 DB에서 읽어오지 않고)메모리에서 데이터를 읽어서 
빨리 리턴할 수 있게하면 DB 부하를 줄일 수 있을 것입니다.  
```
...
메모리에 캐싱하는 방법을 포함하여, 서버 구조적인 해결 방법도 고민해보았습니다. 
아래의 서비스 확장성 부분에서 설명해 보겠습니다. 
```

- 대용량 트래픽
```
트래픽이 많다는 것은, 거의 모든 서버의 부하 발생을 의미합니다.

이 프로젝트의 흐름에서는 로그인 단계에서 먼저 부하가 발생할 것이고, 로그인에 성공하더라도 API 기능 사용에서 
부하로 인한 문제가 발생할 것입니다. (로그인 기능과 API호출 기능이 한 프로젝트에 다 존재하기 때문)

대용량 트래픽을 감당할 수 있는 서버 구성 방법에 대한 생각을 아래의 서비스 확장성 부분에서 작성해보겠습니다. 
```

- 서비스 확장성
```
대용량 트래픽으로 인한 서비스 확장은 어느 한 부분만 확장해서 될 일이 아니라고 생각합니다. 
그래서 전체 서비스를 확장할 수 있는 서버 구조도를 작성해 보았습니다. 
```
...
과 같은 Container Orchestration 도구를 이용해 거의 무한 확장이 가능하다고 합니다. 
...
연결할 수 있습니다. 이때 토큰 검증은 각각의 API 서버에서 담당하게 하여 인증서버의 트래픽 부담을 분산시킬 수 있습니다. 
AWS의 경우 서버 자원의 임계치에 대한 설정으로 API 서버 노드들을 확장할 수 있으므로 대용량 트래픽도 감당할 수 있을 것입니다. 
또한 Async Non-Blocking 방식으로 오픈 API를 요청하므로 노드들의 자원을 효율적으로 사용할 수 있게 됩니다. 

DB 의 경우에는 데이터의 무결성을 유지해야 하므로 확장성에서 불리한 부분이니, 캐싱을 최대한 활용하고 캐싱을 확장
할 수 있는 구조로 설계하면 좋을 것 같습니다.   
```
